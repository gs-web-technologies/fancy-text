...
function Input({ name, type, placeholder, register, error, value='', className='' }) {
    ...
    return (
        <div className="relative z-0 w-full mb-3 group">
            <input
                ...
                {...register(name)}
                id={name}
                className={className ? className : "block py-2.5 px-0 w-full text-sm text-heading bg-transparent border-0 border-b-2 border-default-medium appearance-none focus:outline-none focus:ring-0 focus:border-brand peer"}
                placeholder=" "
                defaultValue = {value}
            />
